...
class Database:
    ...
    def initialize():
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS request_buttons(guild_id INTEGER, channel_id INTEGER, message_id INTEGER PRIMARY KEY, title TEXT, description TEXT, requests JSON, created_at TEXT)")
        cur.execute("CREATE TABLE IF NOT EXISTS guild_settings(guild_id INTEGER PRIMARY KEY, client_role INTEGER, buyer_role INTEGER, admin_role INTEGER, verify_role INTEGER, log_channel INTEGER, request_category INTEGER, slot_category INTEGER, slot_ticket_category INTEGER)")
